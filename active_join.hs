...
instance Monoid (Canvas ()) where
	...
	mappend :: Canvas () ->Canvas () -> Canvas ()
	a `mappend` b = do{
					a;
					b}


join::Active (Canvas ()) ->Active (Canvas ()) ->Active (Canvas ())
join a b= a ->> b

join1 :: [Active (Canvas ())] -> Active (Canvas ())
join1 xs = foldl1 (->>) xs
...
main :: IO ()
main = blankCanvas 3000 $ \ context -> play context $ simulate (toRational 30) $ join1 [newActive1,newActive2,newActive3,newActive4,newActive4a,newActiveHead,newActiveLabel,newActiveLeftSlide,newActiveRightSlide,newActiveSquares,newActivefinalLabel]
